...
class Home extends React.Component {
	...
	_displayDaysName() {
		...
		for (let i = 0; i < 7; i++) {
			daysname.push(
				<Text style={[DefaultStyle.default_text, styles.daysname_text]}>
	...
	_displayDates() {
		...
		for (let week = 0; week < this.getWeeksNums(this.state.date); week++) {
			...
			for (let day = 0; day < 7; day++) {
				...
				if (formated in this.state.events) {
					events = this.state.events[formated]
						.slice(0, 5)
						.map(event => (
							<View
								style={[
				...
				dates.push(
					<TouchableOpacity
						...
						onPressIn={this._changeSelectedDay.bind(this, date)}>
						<Text
							style={[
								DefaultStyle.default_text,
								styles.day_text,
								this.withinMonth(date, this.state.date)
									? {}
									: styles.othermonth_day_text,
								this.sameDate(date, this.state.selectedDate)
									? styles.current_day_text
									: {}
							]}>
							{date.format('DD')}
						</Text>
						<View style={styles.event_days_list}>{events}</View>
					</TouchableOpacity>
				)
		...
		return (
			<View style={styles.main_container}>
				<HeaderDate
					...
					background={this.getFirstEventColor()}
				/>
				...
				<View
					...
					<TouchableOpacity
						onPress={async () => this._changeMonth(-1)}>
						<Image
