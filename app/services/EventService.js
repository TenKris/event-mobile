...
class EventService {
	...
	async get(id) {
		...
		return await this.database
			...
			.then(db =>
				db.executeSql(
					`SELECT event_id as id, name, description, start, end, color FROM Event WHERE event_id = ?;`,
					[id]
				)
			)
			.then(([results]) => {
	...
	async getByDate(start, end) {
		...
		return await this.database
			...
			.then(db =>
				db.executeSql(
					`SELECT event_id as id, name, description, start, end, color FROM Event WHERE start >= ? AND start <= ? ORDER BY start;`,
					[start, end]
				)
			)
			.then(([results]) => {
	...
	async create(event) {
		return await this.database
			...
			.then(db =>
				db.executeSql(
					'INSERT INTO Event (name, description, start, end, color) VALUES (?, ?, ?, ?, ?);',
					[
						event.name,
						event.description,
						event.start.unix(),
						event.end == null ? null : event.end.unix(),
						event.color
					]
				)
			)
