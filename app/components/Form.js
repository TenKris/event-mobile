...
import { StyleSheet, View, ScrollView, Button, TextInput, FlatList, Image, Text, TouchableOpacity, Keyboard } from 'react-native';
                    ...
                    {element.type == "colors" ?
                        <View style={styles.elements_holder}>
                            {element.values.map(value =>
                                <TouchableOpacity onPressIn={() => this.changeEvent(key, value)}>
                                    <View style={[styles.element_color, { backgroundColor: value }, this._borderSelectedColor(value)]}></View>
                                </TouchableOpacity>
                            )}
                        </View>
                        :
                        <TextInput style={[styles.input, element.type == "text_area" ? styles.text_area : {}]} placeholder={element.placeholder || ""} maxLength={element.options != null ? element.options.maxLength || 75 : 75} value={this.state.event[key]} onChangeText={(text) => this.changeEvent(key, text)} {...element.attr} />
                    }
                    <Text style={styles.invalid_message}>{element.error || ""}</Text>
                </View>
            )
        })
        ...
        return (
            <View style={styles.main_container}>
                            ...
                            <View style={[styles.form_group, styles.form_row]}>
                                <View style={[styles.form_group, styles.form_col]}>
                                    ...
                                    <DatePicker
                                        ...
                                        showIcon={false}
                                        is24Hour={true}
                                        onDateChange={(date) => this.changeEvent("start", moment(date, this.defaultFormat))}
                                    />
                                    <Text style={styles.invalid_message}></Text>
                                </View>
                                <View style={[styles.form_group, styles.form_col]}>
                                    <Text style={styles.label}>Date de Fin</Text>
                                    <TextInput style={[styles.input, { textAlign: 'center' }]} placeholder="Choisissez la date" value={this.state.event.end.format("DD/MM/YYYY HH:mm")} />
                                    <Text style={styles.invalid_message}></Text>
                                </View>
                            </View>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                    ...
                    <Button title="valider" style={{ backgroundColor: 'green' }} onPress={() => this.submit(this.state.event)} />
                </View>
            </View>
        );
    }
